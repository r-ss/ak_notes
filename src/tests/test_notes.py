...
def test_note_create_under_default_category(client, alice):
    ...
    note_data_save = data = {"title": f"new_note_{ make_random_string(4) }", "body": "note in default category by Alice"}
...
def test_note_create_under_specific_category(client, alice):
    global note_uuid_save
    global note_data_save
    data = {"title": f"new_nogte_{ make_random_string(4) }", "body": "note in specific category by Alice"}
